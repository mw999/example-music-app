const { setupTestApi } = require("../../../setupTestApi");
...
beforeEach(async () => {
  album = await db.Album.query().insertGraphAndFetch({
    name: "yeezus",
    songs: [
      {
        name: "test",
...
describe("CREATE_ALBUM", () => {
  ...
  test("can create an album", async () => {
    ...
    expect(
      await db.Album.query().where("name", "my new album").first()
    ).toBeTruthy();
  });
...
describe("DELETE_ALBUM", () => {
  ...
  test("can delete an album", async () => {
    ...
    expect(await db.Album.query().where("id", album.id).first()).toBeFalsy();
  });
